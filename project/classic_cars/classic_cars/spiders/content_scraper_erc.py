...
class contentSpiderERC(scrapy.Spider):
    ...
    start_urls = []
    with jsonlines.open("./JSONS/links_erc.jsonl") as f:
        for line in f.iter():
    ...
    def parse(
        ...
        content = response.xpath(
            './/div[@class="product-description"]/div[@class="std"]/p'
        )  # For each quote element in the current page...
        summary = (
            response.xpath('.//div[@class="product-name"]/h1/text()').get() + " \n"
        )
        for p in content:
